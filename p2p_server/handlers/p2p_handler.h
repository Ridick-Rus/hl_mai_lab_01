...
public:
    ...
    // На вход подается строка запроса request. Задача - заполнить выход - response
    {
        ...
        try
        {
            ...
            {
                if (hasSubstr(request.getURI(), "/load")) 
                {
                    if (form.has("id_sender") && form.has("id_receiver"))
                    {
                        int id_sender = stoi(form.get("id_sender")); 
                        int id_receiver = stoi(form.get("id_receiver")); 
                        auto results = database::P2P::load_message(id_sender, id_receiver);
                        Poco::JSON::Array arr;
                        for (auto s : results)
                            arr.add(s.toJSON());
                        response.setStatus(Poco::Net::HTTPResponse::HTTP_OK);
